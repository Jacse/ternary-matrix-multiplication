use std::{cmp::min, simd::Simd, thread};
...
fn at(r: usize, c: usize, cstride: usize) -> usize {
    r + cstride * c
}
...
fn dot16x16(
    ...
    ab[0] = Simd::from_slice(&c[at(0, 0, csc)..(at(0, 0, csc) + 16)]);
    ab[1] = Simd::from_slice(&c[at(0, 1, csc)..(at(0, 1, csc) + 16)]);
    ab[2] = Simd::from_slice(&c[at(0, 2, csc)..(at(0, 2, csc) + 16)]);
    ab[3] = Simd::from_slice(&c[at(0, 3, csc)..(at(0, 3, csc) + 16)]);
    ab[4] = Simd::from_slice(&c[at(0, 4, csc)..(at(0, 4, csc) + 16)]);
    ab[5] = Simd::from_slice(&c[at(0, 5, csc)..(at(0, 5, csc) + 16)]);
    ab[6] = Simd::from_slice(&c[at(0, 6, csc)..(at(0, 6, csc) + 16)]);
    ab[7] = Simd::from_slice(&c[at(0, 7, csc)..(at(0, 7, csc) + 16)]);
    ab[8] = Simd::from_slice(&c[at(0, 8, csc)..(at(0, 8, csc) + 16)]);
    ab[9] = Simd::from_slice(&c[at(0, 9, csc)..(at(0, 9, csc) + 16)]);
    ab[10] = Simd::from_slice(&c[at(0, 10, csc)..(at(0, 10, csc) + 16)]);
    ab[11] = Simd::from_slice(&c[at(0, 11, csc)..(at(0, 11, csc) + 16)]);
    ab[12] = Simd::from_slice(&c[at(0, 12, csc)..(at(0, 12, csc) + 16)]);
    ab[13] = Simd::from_slice(&c[at(0, 13, csc)..(at(0, 13, csc) + 16)]);
    ab[14] = Simd::from_slice(&c[at(0, 14, csc)..(at(0, 14, csc) + 16)]);
    ab[15] = Simd::from_slice(&c[at(0, 15, csc)..(at(0, 15, csc) + 16)]);
    ...
    // TODO: Pack c?
    c[at(0, 0, csc)..(at(0, 0, csc) + 16)].copy_from_slice(&ab[0].to_array());
    c[at(0, 1, csc)..(at(0, 1, csc) + 16)].copy_from_slice(&ab[1].to_array());
    c[at(0, 2, csc)..(at(0, 2, csc) + 16)].copy_from_slice(&ab[2].to_array());
    c[at(0, 3, csc)..(at(0, 3, csc) + 16)].copy_from_slice(&ab[3].to_array());
    c[at(0, 4, csc)..(at(0, 4, csc) + 16)].copy_from_slice(&ab[4].to_array());
    c[at(0, 5, csc)..(at(0, 5, csc) + 16)].copy_from_slice(&ab[5].to_array());
    c[at(0, 6, csc)..(at(0, 6, csc) + 16)].copy_from_slice(&ab[6].to_array());
    c[at(0, 7, csc)..(at(0, 7, csc) + 16)].copy_from_slice(&ab[7].to_array());

    c[at(0, 8, csc)..(at(0, 8, csc) + 16)].copy_from_slice(&ab[8].to_array());
    c[at(0, 9, csc)..(at(0, 9, csc) + 16)].copy_from_slice(&ab[9].to_array());
    c[at(0, 10, csc)..(at(0, 10, csc) + 16)].copy_from_slice(&ab[10].to_array());
    c[at(0, 11, csc)..(at(0, 11, csc) + 16)].copy_from_slice(&ab[11].to_array());
    c[at(0, 12, csc)..(at(0, 12, csc) + 16)].copy_from_slice(&ab[12].to_array());
    c[at(0, 13, csc)..(at(0, 13, csc) + 16)].copy_from_slice(&ab[13].to_array());
    c[at(0, 14, csc)..(at(0, 14, csc) + 16)].copy_from_slice(&ab[14].to_array());
    c[at(0, 15, csc)..(at(0, 15, csc) + 16)].copy_from_slice(&ab[15].to_array());
}
...
pub fn prep10(a: &[i8], b: &[i8]) -> (Vec<u8>, Vec<u8>, Vec<u8>, Vec<u8>) {
...
pub fn matmul10(a_vals: &[u8], a_signs: &[u8], b_vals: &[u8], b_signs: &[u8]) -> Vec<i8> {
    ...
    let c_vertical_parts = c.chunks_exact_mut(nc * m);

    thread::scope(|s| {
        for (nii, c_part) in c_vertical_parts.into_iter().enumerate() {
            let ni = nii * nc;
            s.spawn(move || {
                ...
                for ki in (0..k).step_by(kc) {
                    ...
                    for mi in (0..m).step_by(mc) {
                        ...
                        println!("mi {}, tile m {}", mi, tile_m);
                        print_matrix(c_part, m, 16, m, 1);
                    }
                }
            });
        }
...
mod tests {
    ...
    use super::{matmul10, prep10};

    #[test]
    fn test() {
        test_matmul(|a, b| {
            let (av, asi, bv, bs) = prep10(&a, &b);
            matmul10(&av, &asi, &bv, &bs)
        })
    }
}
