...
pub fn prep8(a: &[i8], b: &[i8]) -> (Vec<u8>, Vec<u8>, Vec<u8>, Vec<u8>) {
...
pub fn matmul8(a_vals: &[u8], a_signs: &[u8], b_vals: &[u8], b_signs: &[u8]) -> Vec<i8> {
...
mod tests {
    ...
    use super::{matmul8, prep8};

    #[test]
    fn test() {
        test_matmul(|a, b| {
            let (av, asi, bv, bs) = prep8(&a, &b);
            matmul8(&av, &asi, &bv, &bs)
        })
    }
}
