...
pub fn prep7(a: &[i8], b: &[i8]) -> (Vec<u8>, Vec<u8>, Vec<u8>, Vec<u8>) {
...
pub fn matmul7(a_vals: &[u8], a_signs: &[u8], b_vals: &[u8], b_signs: &[u8]) -> Vec<i8> {
...
mod tests {
    ...
    use super::{matmul7, prep7};

    #[test]
    fn test() {
        test_matmul(|a, b| {
            let (av, asi, bv, bs) = prep7(&a, &b);
            matmul7(&av, &asi, &bv, &bs)
        })
    }
}
