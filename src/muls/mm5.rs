...
fn pack_b(k: usize, n: usize, b: &[i8], packed: &mut [i8]) {
    let mut offset = 0;
    ...
    for ni in (0..n).step_by(16) {
        ...
        for ki in 0..k {
            ...
            packed[offset] = b[at(ki, ni + 0)];
            packed[offset + 1] = b[at(ki, ni + 1)];
            packed[offset + 2] = b[at(ki, ni + 2)];
            packed[offset + 3] = b[at(ki, ni + 3)];
            packed[offset + 4] = b[at(ki, ni + 4)];
            packed[offset + 5] = b[at(ki, ni + 5)];
            packed[offset + 6] = b[at(ki, ni + 6)];
            packed[offset + 7] = b[at(ki, ni + 7)];
            packed[offset + 8] = b[at(ki, ni + 8)];
            packed[offset + 9] = b[at(ki, ni + 9)];
            packed[offset + 10] = b[at(ki, ni + 10)];
            packed[offset + 11] = b[at(ki, ni + 11)];
            packed[offset + 12] = b[at(ki, ni + 12)];
            packed[offset + 13] = b[at(ki, ni + 13)];
            packed[offset + 14] = b[at(ki, ni + 14)];
            packed[offset + 15] = b[at(ki, ni + 15)];
...
fn pack_a(k: usize, m: usize, a: &[i8], packed: &mut [i8]) {
    let mut offset = 0;
...
pub fn matmul5(a: &[i8], b: &[i8]) -> Vec<i8> {
    ...
    let mut packed_a = vec![0_i8; mc * kc];
    let mut packed_b = vec![0_i8; kc * nc];
